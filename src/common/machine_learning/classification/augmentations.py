...
def create_single_augmentation(name: str, args: Dict[str, Any]) -> AugmentationBase2D:
    """
    ...
    if augmentation_class is None:
        ...
        error_message = (
            f"Augmentation '{name}' is not implemented. " f"Available augmentations are: {available_augmentations}"
        )
...
def create_augmentation_sequence(config: AugmentationConfig) -> Sequential:
    """
    ...
    augmentations = [create_single_augmentation(name, args) for name, args in zip(config.names, config.arguments)]
